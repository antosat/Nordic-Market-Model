...
for i = 1:size(zones,2)
    ...
    if strcmp(Options.optimization,'EconomicDispatch')
        ...
        PowerBalance = sum((TestCase.(char(zones{i})).Load.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Load.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Load.Pmax)... Load
        ...
        if isfield(TestCase.(char(zones{i})),'Wind')
            PowerBalance = PowerBalance - sum((TestCase.(char(zones{i})).Wind.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Wind.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Wind.Pmax); % Wind
        end
        if isfield(TestCase.(char(zones{i})),'Solar')
            PowerBalance = PowerBalance - sum((TestCase.(char(zones{i})).Solar.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Solar.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Solar.Pmax); % Solar
        end
        if Options.loadshed == 1
    ...
    elseif strcmp(Options.optimization,'DC_OPF')
        ...
        PowerInjections = -(TestCase.(char(zones{i})).Load.IncidenceMatrix)*((TestCase.(char(zones{i})).Load.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Load.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Load.Pmax); %Load
        ...
        if isfield(TestCase.(char(zones{i})),'Wind')
            %Inclusion of wind
            PowerInjections = PowerInjections + (TestCase.(char(zones{i})).Wind.IncidenceMatrix)*((TestCase.(char(zones{i})).Wind.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Wind.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Wind.Pmax); %Wind
            PowerBalance = PowerBalance - sum((TestCase.(char(zones{i})).Wind.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Wind.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Wind.Pmax); %Wind
        end
        if isfield(TestCase.(char(zones{i})),'Solar')
            %Inclusion of solar
            PowerInjections = PowerInjections + (TestCase.(char(zones{i})).Solar.IncidenceMatrix)*((TestCase.(char(zones{i})).Solar.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Solar.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Solar.Pmax); % Solar
            PowerBalance = PowerBalance - sum((TestCase.(char(zones{i})).Solar.CoefficientIncidenceMatrix*TestCase.(char(zones{i})).Solar.Coefficients(:,Options.hour)).*TestCase.(char(zones{i})).Solar.Pmax); %Solar
        end
        ...
        if isfield(TestCase.(char(zones{i})).Network,'ACsystem')
            if strcmp(Options.marketclearing,'zonal_ATC') 
                Constraints = [ Constraints
                    % AC line limits
                    ( evalin('base',strcat('f_ac_',char(zones{i}))) >= TestCase.(char(zones{i})).Network.ACsystem.Capacities.Lower(:,Options.hour) ):strcat('MuLb_fac_',char(zones{i}))
                    ( evalin('base',strcat('f_ac_',char(zones{i}))) <= TestCase.(char(zones{i})).Network.ACsystem.Capacities.Upper(:,Options.hour) ):strcat('MuUb_fac_',char(zones{i}))
                    ];
        ...
        if isfield(TestCase.(char(zones{i})).Network,'HVDCinternal')
            if strcmp(Options.marketclearing,'zonal_PTDF') || strcmp(Options.marketclearing,'zonal_ATC')
                Constraints = [ Constraints
                    ...
                    ( evalin('base',strcat('f_hvdc_',char(zones{i}))) >= TestCase.(char(zones{i})).Network.HVDCinternal.Capacities.Lower(:,Options.hour) ):strcat('MuLb_fhvdc_',char(zones{i}))
                    ( evalin('base',strcat('f_hvdc_',char(zones{i}))) <= TestCase.(char(zones{i})).Network.HVDCinternal.Capacities.Upper(:,Options.hour) ):strcat('MuUb_fhvdc_',char(zones{i}))
                ];
            end
        end
        if isfield(TestCase.(char(zones{i})).Network,'HVDCinterconnectors')
            if strcmp(Options.marketclearing,'zonal_PTDF') || strcmp(Options.marketclearing,'zonal_ATC')
                Constraints = [ Constraints
                    ...
                    ( evalin('base',strcat('f_inter_',char(zones{i}))) >= TestCase.(char(zones{i})).Network.HVDCinterconnectors.Capacities.Lower(:,Options.hour) ):strcat('MuLb_finter_',char(zones{i}))
                    ( evalin('base',strcat('f_inter_',char(zones{i}))) <= TestCase.(char(zones{i})).Network.HVDCinterconnectors.Capacities.Upper(:,Options.hour) ):strcat('MuUb_finter_',char(zones{i}))
                    ];
            end
        end
